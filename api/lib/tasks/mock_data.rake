...
namespace :mock_data do
  desc 'Setup a mock user with 4 devices, send sensor data every minute with 5s initial delay, and fetch commands every 5 seconds'
  task full_setup: :environment do
    ...
    # Step 1: Create User
    user = User.create!(
      email: "mockuser_#{rand(1000..9999)}@example.com",
      password: 'password',
      role: :pro,
      timezone: "UTC" # Updated to valid timezone
    )
    puts "✅ Created User: #{user.email} (Timezone: #{user.timezone})"

    # Step 2: Assign Professional Subscription
    plan = Plan.find_by(name: 'Professional')
    subscription = Subscription.create!(user: user, plan: plan, status: 'active', current_period_start: Time.current, current_period_end: 1.month.from_now)
    puts '✅ Assigned Professional Subscription'
    ...
    4.times do |i|
      ...
      SubscriptionDevice.create!(subscription: subscription, device: device, monthly_cost: 10.00)
      puts "✅ Activated Device ##{i + 1}: #{device.name}"
    end

    puts '🚀 All devices activated. Starting sensor data simulation and command polling...'
    devices = user.devices
    sensors = devices.flat_map(&:device_sensors)
    puts "✅ Found #{sensors.count} sensors across 4 devices"
  ...
  # Rest of the file (private methods) remains unchanged...
  private
  ...
  def send_sensor_data(device, state)
    ...
    puts "[#{Time.current}] ❌ Device #{device.id} - API Error: #{response.code} - #{response.body}" unless response.is_a?(Net::HTTPSuccess)
  rescue StandardError => e
