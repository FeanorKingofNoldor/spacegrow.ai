# app/services/plan_change_analyzer.rb
class PlanChangeAnalyzer
  ...
  def device_selection_data(device)
    ...
    
    {
      ...
      priority_reason: get_priority_reason(device)
    }
  end
  
  def get_priority_reason(device)
    one_week_ago = 1.week.ago.utc
    one_day_ago = 1.day.ago.utc
    
    return 'Offline for over 1 week' if device.last_connection.nil? || device.last_connection < one_week_ago
    return 'Offline for over 1 day' if device.last_connection < one_day_ago
    return 'Oldest device' # Default reason
  end
