  ...
  class << self
    ...
    def change_plan_with_strategy(user:, target_plan:, target_interval: 'month', strategy:, selected_device_ids: [])
      ...
      when 'hibernate_excess'  # ✅ NEW: Added missing strategy
        execute_change_with_hibernation(user, target_plan, target_interval, selected_device_ids)
      else
    ...
    def execute_immediate_change_with_selection(user, target_plan, target_interval, selected_device_ids)
      ...
      unselected_devices = user.devices.operational.where.not(id: selected_device_ids)
      
      # ✅ ENHANCED: Use hibernation instead of disabling
      hibernated_devices = []
      unselected_devices.each do |device|
        device.hibernate!(reason: 'plan_change')
        hibernated_devices << device
      end

      result[:hibernated_devices] = hibernated_devices.count
      result[:message] = "Plan changed successfully. #{hibernated_devices.count} devices hibernated."
    ...
    # ✅ NEW: Added hibernate_excess strategy implementation
    def execute_change_with_hibernation(user, target_plan, target_interval, selected_device_ids)
      old_plan = user.active_subscription&.plan || user.subscription&.plan
      subscription = user.active_subscription || user.subscription
      
      # Validate that the selected devices exist and belong to the user
      devices_to_hibernate = user.devices.operational.where(id: selected_device_ids)
      
      if devices_to_hibernate.count != selected_device_ids.count
        invalid_ids = selected_device_ids - devices_to_hibernate.pluck(:id)
      ...
      # Hibernate the selected devices
      hibernated_devices = []
      devices_to_hibernate.each do |device|
        device.hibernate!(reason: 'plan_change_hibernation')
        hibernated_devices << {
          id: device.id,
          name: device.name
        }
...

      {
        status: 'completed',
        subscription: subscription,
        hibernated_devices: hibernated_devices.count,
        hibernation_summary: {
          hibernated_count: hibernated_devices.count,
          grace_period_days: 7,
          can_wake_immediately: true
        },
        message: "Plan changed with #{hibernated_devices.count} devices hibernated (can be woken anytime)"
      }
    end
    ...
    def handle_subscription_change(user, old_plan, new_plan)
      ...
      if new_plan.device_limit >= current_active_devices
        # Wake up hibernating devices if there's room
        hibernating_devices = user.devices.hibernating.limit(new_plan.device_limit - current_active_devices)
        hibernating_devices.each(&:wake_up!)
      else
