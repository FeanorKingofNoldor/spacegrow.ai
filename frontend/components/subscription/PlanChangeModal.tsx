...
interface PlanChangeModalProps {
  ...
  onClose: () => void;
  onComplete?: () => void;
  preselectedPlan?: Plan | undefined; // ✅ This is already correct
}
...
export function PlanChangeModal({ 
  ...
  // ✅ FIXED: Changed from Plan | null to Plan | undefined
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // ✅ FIXED: Changed from || null to || undefined
  ...
  const handleStrategySelect = async (strategy: ChangeStrategy) => {
    ...
    // If strategy requires device selection, fetch devices
    if (strategy.type === 'immediate_with_selection') {
      ...
      try {
        ...
        // Auto-select recommended devices to disable
        const recommended = devices
          .filter(d => d.recommendation === 'recommended_to_disable')
          .map(d => d.id);
        
        const needed = preview?.device_impact.target_device_limit || 0;
        const keepDevices = devices
          .filter(d => d.recommendation === 'keep_active')
          .slice(0, needed)
          .map(d => d.id);
        
        setSelectedDevices(keepDevices);
